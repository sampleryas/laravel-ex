...
class Stocktwits {
  ...
  function getStream($id = "home") {
    ...
    $profileApiLocation = $this->secure_base_url .'streams/symbol/BTC.X.json';
    $curl = curl_init();
    curl_setopt_array($curl, array(
      CURLOPT_RETURNTRANSFER => 1,
      CURLOPT_URL => $profileApiLocation,
...
// $asd = new Stocktwits(null, null, null);
// $res = $asd->getStream('home');

// // print_r($res->messages);
// foreach($res->messages as $msg){
//   if($msg->entities->sentiment != null){
//     echo $msg->entities->sentiment->basic ."\n";
//   }
// }